...
/* ---------------- BLOG PREVIEW ---------------- */
const posts = [
  { tag: "Design", title: "10 UI trends defining premium web in 2026", read: "5 min", color: "from-violet/30 to-magenta/20" },
  { tag: "AI", title: "How we ship AI features that users actually trust", read: "7 min", color: "from-cyan/30 to-violet/20" },
  { tag: "Startups", title: "From idea to MVP in six weeks: our playbook", read: "6 min", color: "from-magenta/30 to-cyan/20" },
];

export function BlogPreview() {
  return (
    <Section id="blog">
      <SectionHeading eyebrow="Blog" title={<>Latest <span className="text-gradient">articles</span></>} />
      <div className="grid gap-6 md:grid-cols-3">
        {posts.map((p, i) => (
          <motion.article
            ...
            whileInView="show"
            viewport={{ once: true }}
            className="group overflow-hidden rounded-3xl glass transition-all hover:-translate-y-1.5 hover:shadow-glow"
          >
            <div className={`aspect-[16/10] bg-gradient-to-br ${p.color}`} />
            <div className="p-6">
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                <span className="rounded-full bg-primary/15 px-2.5 py-1 font-medium text-primary">{p.tag}</span>
              ...
              <button className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-primary">
                Read More <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
              </button>
            </div>
          </motion.article>
        ))}
